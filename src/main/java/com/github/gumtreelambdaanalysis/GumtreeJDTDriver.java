...
public class GumtreeJDTDriver
{
    ...
    public GumtreeJDTDriver(String sourceCode, List<PositionTuple> positionTupleList, boolean findBadLambda)
    {
        ...
        if (findBadLambda)
        {
            LambdaFinder lambdaFinder = new LambdaFinder(cu, 0, Integer.MAX_VALUE, positionTupleList);
...
//    }
    ...
    private class LambdaFinder extends ASTVisitor
    {
        ...
        private LambdaFinder(CompilationUnit compilationUnit, int startLine, int endLine, List<PositionTuple> positionTupleList)
        {
            cu = compilationUnit;
            this.startLine = startLine;
            this.endLine = endLine;
